...
router.get("/image/:fileid", (req, res) => {
    try {
        let data;
        minioClient.getObject("stories", req.params.fileid, (err, objStream) => {
            if(err) {
...
router.post("/image", upload.single('image'), (req, res) => {
    try { 
        var filePath = "/home/iit/Documents/Github/fakebook/api/"+ req.file.path;
        ...
        minioClient.fPutObject("stories", fileName, filePath, metaData, function(err, etag) {
            if (err){ 
                return res.status(402).send({ 
                    message: "Error at saving image data !!!", 
                    error: err
                });
            }
            res.status(200).send({ 
        ...
        res.status(500).send({ 
            message: "Internal Server Error at posting image" 
        });
    }
});
...
router.post("/", (req, res) => {
    try {
        ...
        new Story({
            ...
            if(err) 
                res.status(402).send({ 
                    message: "Error at saving story id data !!!", 
                    error: err
                });
            else 
                res.status(200).send({ 
                    message: "Story id saved successfully ..." 
                });
        }); 
    } catch (error) {
        res.status(500).send({ 
            message: "Internal Server Error at storing story id" 
        });
    }
})
